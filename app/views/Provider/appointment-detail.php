<?php
// app/views/Provider/appointment-detail.php
require_once __DIR__ . '/../../../config/database.php';
$db     = Database::getInstance();
$userId = (int)($_SESSION['user_id'] ?? 0);

$navUser = $db->prepare("SELECT u.first_name, u.last_name, u.email, pp.profile_photo AS nav_photo, pp.business_name, c.name AS category_name FROM tbl_users u LEFT JOIN tbl_provider_profiles pp ON pp.user_id = u.id LEFT JOIN tbl_categories c ON pp.category_id = c.id WHERE u.id = ?");
$navUser->execute([$userId]);
$navRow       = $navUser->fetch();
$firstName    = htmlspecialchars($navRow['first_name'] ?? 'Provider');
$provFullName = htmlspecialchars(trim(($navRow['first_name'] ?? '') . ' ' . ($navRow['last_name'] ?? '')) ?: 'Provider');
$bizName      = htmlspecialchars($navRow['business_name'] ?? $firstName);
$bizCategory  = htmlspecialchars($navRow['category_name'] ?? 'Service Provider');
...
<head>
  ...
  <title>QuickBook — Appointment #<?= str_pad((int)$booking['id'], 4, '0', STR_PAD_LEFT) ?></title>
  ...
  <link rel="stylesheet" href="<?= BASE_URL ?>assets/css/provider_appointment_detail.css">
  ...
  <div class="pv-hero-inner">
    <div>
      <p class="pv-hero-eyebrow">
        ...
        Appointment #<?= str_pad((int)$booking['id'], 4, '0', STR_PAD_LEFT) ?>
      </p>
    ...
    <a href="<?= BASE_URL ?>provider/appointments" class="pv-hero-chip">
  ...
  <div class="bd-grid">
    ...
    <div class="bd-main">
      ...
      <div class="pv-card bd-card">
        ...
        <div style="display:flex;align-items:center;gap:1rem;margin-bottom:1.25rem">
          <div style="width:56px;height:56px;border-radius:50%;background:linear-gradient(135deg,var(--gold-dim),var(--gold));color:#fff8e8;font-family:var(--font-display);font-weight:700;font-size:1.1rem;display:flex;align-items:center;justify-content:center;flex-shrink:0;box-shadow:0 0 0 3px var(--gold-border),0 2px 12px rgba(201,168,76,.25);overflow:hidden;">
            <?php if (!empty($booking['customer_avatar'])): ?>
              <img src="<?= htmlspecialchars($booking['customer_avatar']) ?>" alt="" style="width:100%;height:100%;object-fit:cover;display:block;">
            <?php else: ?>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:.75rem 1rem">
          <div style="grid-column:1/-1;display:flex;align-items:flex-start;gap:.65rem;padding:.7rem .9rem;background:rgba(255,255,255,.55);border:1px solid var(--gold-border);border-radius:10px">
            ...
            <div>
              <div style="font-size:.62rem;text-transform:uppercase;letter-spacing:.07em;color:var(--text-dim);margin-bottom:.2rem">Email</div>
          ...
          <div style="display:flex;align-items:flex-start;gap:.65rem;padding:.7rem .9rem;background:rgba(255,255,255,.55);border:1px solid var(--gold-border);border-radius:10px">
            ...
            <div>
              <div style="font-size:.62rem;text-transform:uppercase;letter-spacing:.07em;color:var(--text-dim);margin-bottom:.2rem">Phone</div>
          ...
          <div style="display:flex;align-items:flex-start;gap:.65rem;padding:.7rem .9rem;background:rgba(255,255,255,.55);border:1px solid var(--gold-border);border-radius:10px">
            ...
            <div>
              <div style="font-size:.62rem;text-transform:uppercase;letter-spacing:.07em;color:var(--text-dim);margin-bottom:.2rem">Gender</div>
              <div style="font-size:.88rem;color:var(--text-primary);font-weight:500">
          ...
          <div style="display:flex;align-items:flex-start;gap:.65rem;padding:.7rem .9rem;background:rgba(255,255,255,.55);border:1px solid var(--gold-border);border-radius:10px">
            ...
            <div>
              <div style="font-size:.62rem;text-transform:uppercase;letter-spacing:.07em;color:var(--text-dim);margin-bottom:.2rem">Date of Birth</div>
              <div style="font-size:.88rem;color:var(--text-primary);font-weight:500">
                <?php if ($custDob): ?>
                  <?= htmlspecialchars($custDob) ?>
                  <?php if ($custAge !== null): ?><span style="font-size:.75rem;color:var(--text-dim);margin-left:.4rem">(<?= $custAge ?> yrs)</span><?php endif; ?>
                <?php else: ?>
      ...
      <div class="pv-card bd-card" style="border-color:rgba(251,191,36,.35);background:rgba(251,191,36,.05)">
        <div class="bd-section-title" style="color:#fbbf24"><i class="fa-solid fa-house-chimney-medical"></i> Home Service Address</div>
        <div style="display:flex;align-items:flex-start;gap:1rem;margin-top:.25rem">
          ...
          <div style="flex:1">
            <div style="font-size:1rem;font-weight:700;color:var(--text-primary);line-height:1.5;margin-bottom:.35rem"><?= htmlspecialchars($custAddr) ?></div>
            <a href="https://www.google.com/maps/search/<?= urlencode($custAddr) ?>" target="_blank" rel="noopener noreferrer"
               style="display:inline-flex;align-items:center;gap:.4rem;padding:.38rem .8rem;background:rgba(251,191,36,.12);border:1px solid rgba(251,191,36,.3);border-radius:999px;font-size:.73rem;font-weight:700;color:#fbbf24;text-decoration:none">
              <i class="fa-solid fa-map"></i> Open in Google Maps
            </a>
            <div style="font-size:.72rem;color:var(--text-dim);margin-top:.5rem"><i class="fa-solid fa-circle-info"></i> Travel to this address to perform the service</div>
          </div>
        </div>
      </div>
      ...
      <div class="pv-card bd-card" style="border-color:rgba(244,63,94,.25);background:rgba(244,63,94,.05)">
        <div class="bd-section-title" style="color:#f43f5e"><i class="fa-solid fa-triangle-exclamation"></i> Home Service Address Missing</div>
        <p style="font-size:.84rem;color:var(--text-muted);margin:.5rem 0 0">This booking is marked as Home Service but the customer did not provide a service address. Please contact the customer to confirm the location.</p>
        <?php if ($custProfAddr): ?>
        <div style="margin-top:.75rem;padding:.7rem .9rem;background:rgba(255,255,255,.04);border:1px solid var(--border);border-radius:10px;font-size:.84rem;color:var(--text-primary)">
          <div style="font-size:.62rem;text-transform:uppercase;letter-spacing:.07em;color:var(--text-dim);font-family:var(--font-mono);margin-bottom:.25rem">Profile home address (reference)</div>
      ...
      <div class="pv-card bd-card">
        ...
        <div class="bd-detail-grid">
          <div class="bd-detail-item"><div class="bd-detail-label">Date</div><div class="bd-detail-val"><?= $bookDateShort ?></div></div>
          <div class="bd-detail-item"><div class="bd-detail-label">Time</div><div class="bd-detail-val"><?= $bookTime ?? '—' ?></div></div>
          <?php if ($duration): ?>
          <div class="bd-detail-item"><div class="bd-detail-label">Duration</div><div class="bd-detail-val">⏱ <?= $duration ?></div></div>
          <?php endif; ?>
          <div class="bd-detail-item">
            <div class="bd-detail-label">Location Type</div>
            <div class="bd-detail-val" style="color:<?= $locInfo['color'] ?>"><i class="fa-solid <?= $locInfo['icon'] ?>"></i> <?= $locInfo['label'] ?></div>
          </div>
          <div class="bd-detail-item"><div class="bd-detail-label">Booked On</div><div class="bd-detail-val" style="font-size:.8rem"><?= $createdAt ?></div></div>
          <div class="bd-detail-item"><div class="bd-detail-label">Booking ID</div><div class="bd-detail-val bd-mono">#<?= str_pad((int)$booking['id'], 4, '0', STR_PAD_LEFT) ?></div></div>
        </div>
        ...
        <div style="display:flex;align-items:flex-start;gap:.7rem;padding:.85rem 1rem;border-radius:12px;border:1px solid rgba(74,222,128,.2);background:rgba(74,222,128,.07);margin-top:1rem">
          ...
          <div>
            <div style="font-size:.7rem;font-family:var(--font-mono);letter-spacing:.08em;text-transform:uppercase;color:#4ade80;margin-bottom:.25rem">Shop Address</div>
            ...
            <a href="https://www.google.com/maps/search/<?= urlencode($shopAddr) ?>" target="_blank" rel="noopener noreferrer"
               style="display:inline-flex;align-items:center;gap:.4rem;padding:.32rem .75rem;background:rgba(22,163,74,.10);border:1px solid rgba(22,163,74,.25);border-radius:999px;font-size:.72rem;font-weight:700;color:var(--green);text-decoration:none">
              <i class="fa-solid fa-map"></i> Open in Google Maps
            </a>
            <div style="font-size:.72rem;color:var(--text-dim);margin-top:.4rem"><i class="fa-solid fa-circle-info"></i> Customer will come to your shop</div>
          </div>
        </div>
    ...
    <div class="bd-sidebar">
      ...
      <div class="pv-card bd-card">
        ...
        <?php
          ...
          $payIcons = [
            'cash'=>'<i class="fa-solid fa-money-bill-wave"></i>','gcash'=>'<i class="fa-solid fa-mobile-screen"></i>',
            'paymaya'=>'<i class="fa-solid fa-mobile-screen"></i>','card'=>'<i class="fa-solid fa-credit-card"></i>',
            'credit_card'=>'<i class="fa-solid fa-credit-card"></i>','debit_card'=>'<i class="fa-solid fa-credit-card"></i>',
        ...
        <?php endif; ?>
        <div class="bd-price-row"><span>Amount</span><span class="bd-price-val">₱<?= number_format($servicePrice, 2) ?></span></div>
        <?php if ($isHomeService && $homeServiceFee > 0): ?>
        <div class="bd-price-row"><span>Home service fee</span><span class="bd-price-val">₱<?= number_format($homeServiceFee, 2) ?></span></div>
        <?php endif; ?>
        <div class="bd-price-divider"></div>
          ...
          <?php else: ?>
            ...
            <div class="bd-timeline-step <?= $done ? 'done' : 'muted' ?> <?= $active ? 'active' : '' ?>">
              ...
              <div class="bd-tl-content">
                ...
                <?php if ($active): ?><div style="font-size:.72rem;color:var(--text-dim);margin-top:.15rem"><?= $info['sub'] ?></div><?php endif; ?>
              </div>
            </div>
            <?php endforeach; ?>
          <?php endif; ?>
      ...
      <div class="pv-card bd-card">
        ...
        <div class="bd-actions">
          <?php if ($canStart): ?>
          <form method="POST" action="<?= BASE_URL ?>provider/appointments/<?= (int)$booking['id'] ?>">
          ...
          <?php endif; ?>
          <button type="button" class="cd-btn cd-btn--delete" onclick="openDeleteModal()"><i class="fa-solid fa-xmark"></i> Cancel &amp; Delete Order</button>
          <button type="button" class="cd-btn cd-btn--resched" onclick="openReschedModal()"><i class="fa-solid fa-rotate-right"></i> Suggest Reschedule</button>
        </div>
      </div>
      <?php endif; ?>

      <div class="bd-actions">
        <a href="<?= BASE_URL ?>provider/appointments" class="cd-btn cd-btn--ghost"><i class="fa-solid fa-arrow-left"></i> All Appointments</a>
      </div>
  ...
  <div class="pv-modal pv-modal--delete">
    ...
    <div class="modal-centered-header">
      <div class="modal-icon-ring modal-icon-ring--red">
        <svg viewBox="0 0 24 24" fill="none" width="26" height="26"><path d="M12 8v5M12 15.5v.5" stroke="#FB7185" stroke-width="2.2" stroke-linecap="round"/></svg>
      </div>
      <h2 class="modal-title" id="deleteModalTitle">Cancel &amp; Delete Order</h2>
      <p class="modal-sub">You are about to cancel the booking for <strong><?= htmlspecialchars(($booking['customer_first'] ?? '') . ' ' . ($booking['customer_last'] ?? '')) ?></strong> (<em><?= htmlspecialchars($booking['service_name'] ?? '') ?></em>).<br>The customer will be <span class="hl-red">immediately notified</span>. This action <span class="hl-red">cannot be undone.</span></p>
    </div>
    <form method="POST" action="<?= BASE_URL ?>provider/appointments/<?= (int)$booking['id'] ?>">
      <input type="hidden" name="action" value="delete">
      <label class="modal-field-label" for="delReason">Reason for cancellation <span class="modal-required">* required</span></label>
      <textarea id="delReason" name="reason" class="pv-textarea" placeholder="e.g. Schedule conflict, Equipment issue…" maxlength="400" required></textarea>
      <div class="modal-char-count"><span id="delCharCount">0</span> / 400</div>
      <div class="modal-foot">
        <button type="submit" class="modal-btn modal-btn--red" id="delSubmitBtn" disabled>Yes, Cancel</button>
  ...
  <div class="pv-modal pv-modal--confirm">
    ...
    <div class="modal-centered-header">
      <div class="modal-icon-ring modal-icon-ring--green">
        <svg viewBox="0 0 24 24" fill="none" width="26" height="26"><path d="M5 12l5 5L19 7" stroke="#4ADE80" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/></svg>
      </div>
      <h2 class="modal-title" id="confirmModalTitle">Confirm Booking</h2>
      <p class="modal-sub">You are about to <span class="hl-green">confirm</span> the booking for <strong><?= htmlspecialchars(($booking['customer_first'] ?? '') . ' ' . ($booking['customer_last'] ?? '')) ?></strong> (<em><?= htmlspecialchars($booking['service_name'] ?? '') ?></em>). The customer will be notified immediately.</p>
    </div>
    <form method="POST" action="<?= BASE_URL ?>provider/appointments/<?= (int)$booking['id'] ?>">
      <input type="hidden" name="action" value="confirm">
      <div class="modal-foot">
        <button type="submit" class="modal-btn modal-btn--green">Yes, Confirm</button>
  ...
  <div class="pv-modal pv-modal--resched">
    ...
    <div class="modal-centered-header">
      <div class="modal-icon-ring modal-icon-ring--amber">
        <svg viewBox="0 0 24 24" fill="none" width="26" height="26"><path d="M21 12A9 9 0 1 1 12 3" stroke="#F59E0B" stroke-width="2" stroke-linecap="round"/><path d="M21 3v5h-5" stroke="#F59E0B" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
      </div>
      <h2 class="modal-title" id="reschedModalTitle">Suggest Reschedule</h2>
      <p class="modal-sub">Suggest a new schedule for <strong><?= htmlspecialchars(($booking['customer_first'] ?? '') . ' ' . ($booking['customer_last'] ?? '')) ?></strong> (<em><?= htmlspecialchars($booking['service_name'] ?? '') ?></em>).<br><span class="modal-sub-note">Current: <span class="hl-amber"><?= $bookDate ?><?= $bookTime ? ' · ' . $bookTime : '' ?></span></span></p>
    </div>
    <form method="POST" action="<?= BASE_URL ?>provider/appointments/<?= (int)$booking['id'] ?>">
      <input type="hidden" name="action" value="reschedule">
      <div class="resch-row">
        <div class="resch-field">
          <label class="modal-field-label" for="reschedDate">Suggested Date <span class="modal-required">*</span></label>
          <input type="date" id="reschedDate" name="suggested_date" class="pv-input" required>
        </div>
        <div class="resch-field">
          <label class="modal-field-label" for="reschedTime">Suggested Time <span class="modal-required">*</span></label>
          <input type="time" id="reschedTime" name="suggested_time" class="pv-input" required>
        </div>
      </div>
      <label class="modal-field-label" for="reschedNote" style="display:block;margin-top:.85rem">Reason / Note <span class="modal-required">*</span></label>
      <textarea id="reschedNote" name="resched_reason" class="pv-textarea" placeholder="e.g. I have a conflict at the original time…" maxlength="500" required></textarea>
      <div class="modal-char-count"><span id="reschedCharCount">0</span> / 500</div>
      <div class="modal-foot">
        <button type="submit" class="modal-btn modal-btn--amber" id="reschedSubmitBtn" disabled>Send Suggestion</button>
      </div>
    </form>
  </div>
</div>
...
<script>
function openConfirmModal()  { document.getElementById('confirmModal').classList.add('is-open'); }
...
function closeDeleteModal() { document.getElementById('deleteModal').classList.remove('is-open'); }
...
function closeReschedModal() { document.getElementById('reschedModal').classList.remove('is-open'); }
document.getElementById('reschedModal').addEventListener('click', function(e){ if(e.target===this) closeReschedModal(); });
function validateReschedForm(){
  var d=document.getElementById('reschedDate').value.trim(),
      t=document.getElementById('reschedTime').value.trim(),
      n=document.getElementById('reschedNote').value.trim();
  document.getElementById('reschedSubmitBtn').disabled=!(d&&t&&n.length>=5);
}
document.getElementById('reschedNote').addEventListener('input',function(){ document.getElementById('reschedCharCount').textContent=this.value.length; validateReschedForm(); });
...
document.addEventListener('keydown',function(e){ if(e.key==='Escape'){ closeConfirmModal(); closeDeleteModal(); closeReschedModal(); } });
...
(function(){
  ...
  if(trigger&&dropdown){
    trigger.addEventListener('click',function(e){ e.stopPropagation(); var open=dropdown.classList.toggle('is-open'); trigger.setAttribute('aria-expanded',open); });
