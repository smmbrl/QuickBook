...
<div class="pv-discovery-strip" role="region" aria-label="Quick discovery filters">
  ...
  <?php
    $chipDefs = [
      ''         => ['icon' => '✦', 'label' => 'All Providers',     'dot' => ''],
      'top'      => ['icon' => '★', 'label' => 'Top Rated',         'dot' => 'dot-gold'],
      'home'     => ['icon' => '🏠', 'label' => 'Home Service',      'dot' => 'dot-green'],
      'verified' => ['icon' => '✔', 'label' => 'Verified',          'dot' => ''],
    ];
    foreach ($chipDefs as $val => $chip):
      $isActive = $quickFilter === $val;
  ...
  ?>
  <a href="<?= $href ?>"
     class="pv-chip <?= $isActive ? 'is-active' : '' ?>"
     aria-pressed="<?= $isActive ? 'true' : 'false' ?>">
    <?php if ($chip['dot']): ?>
      <span class="pv-chip-dot <?= $chip['dot'] ?>"></span>
    <?php else: ?>
      <span class="pv-chip-icon"><?= $chip['icon'] ?></span>
    <?php endif; ?>
    <?= $chip['label'] ?>
  </a>
  <?php endforeach; ?>
        ...
        <div class="pv-stype-menu" id="stypeMenu" role="listbox">
          <?php
            $stypeOptions = [
              ...
              'flexible' => ['label'=>'Flexible',     'desc'=>'Multiple options',          'icon'=>''],
            ];
      ...
      <form method="GET" action="<?= BASE_URL ?>browse">
        ...
        <div class="pv-sort-wrap">
          ...
          <select name="sort" class="pv-sort-select" onchange="this.form.submit()" aria-label="Sort providers">
            ...
            <option value="price_lo" <?= $sortBy==='price_lo' ? 'selected':'' ?>>Price: Low → High</option>
            <option value="price_hi" <?= $sortBy==='price_hi' ? 'selected':'' ?>>Price: High → Low</option>
            <option value="name"     <?= $sortBy==='name'     ? 'selected':'' ?>>A – Z</option>
          </select>
          <svg class="pv-sort-chev" viewBox="0 0 10 6" fill="none" stroke="currentColor"
               stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"
               width="10" height="6" aria-hidden="true"><path d="M1 1l4 4 4-4"/></svg>
        </div>
      </form>
  ...
  <div class="pv-service-grid" role="list">
    ...
    <a href="<?= BASE_URL ?>providers/<?= (int)$p['profile_id'] ?>"
      ...
      <div class="pv-svc-cover">
        <?php if ($coverImg): ?>
          <img src="<?= htmlspecialchars($coverImg) ?>"
               alt="<?= htmlspecialchars($p['business_name']) ?> cover"
               class="pv-svc-cover-img" loading="lazy">
        <?php else: ?>
          <div class="pv-svc-cover-placeholder">
            <?= catSvg($slug, $catIconMap, $allIcon) ?>
          </div>
        <?php endif; ?>
        ...
        <div class="pv-svc-cover-badges">
          <?php if ($openStatus === true): ?>
            <span class="pv-svc-status open">
              <span class="pv-svc-status-dot"></span>Open Now
            </span>
          <?php elseif ($openStatus === false): ?>
            <span class="pv-svc-status closed">
              <span class="pv-svc-status-dot"></span>Closed
            </span>
          <?php endif; ?>
          <?php if ($isVerified): ?>
      ...
      <div class="pv-svc-body">
        ...
        <?php endif; ?>
        <?php if ($svcCount > 0): ?>
          <div class="pv-svc-chips-row">
            <span class="pv-svc-mini-chip"><?= $svcCount ?> service<?= $svcCount!==1?'s':'' ?></span>
          </div>
        <?php endif; ?>
      </div>
      ...
      <div class="pv-svc-gallery">
        <?php foreach ($galleryImgs as $imgUrl): ?>
          <img src="<?= htmlspecialchars($imgUrl) ?>"
               class="pv-svc-gallery-img" alt="Work photo" loading="lazy">
        <?php endforeach; ?>
        <?php if ($extraCount > 0): ?>
          <div class="pv-svc-gallery-more">
            <span>+<?= $extraCount ?></span>
            <span style="font-size:.5rem;opacity:.6;">more</span>
          </div>
        <?php endif; ?>
      </div>
      <?php endif; ?>
