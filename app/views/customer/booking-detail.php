  ...
  <div class="pv-nav-inner">
    ...
    <div class="pv-nav-end">
      ...
      <div class="pv-nav-av" aria-hidden="true">
        <?php
          $stAv2 = $db->prepare("SELECT avatar_url FROM tbl_users WHERE id = ? LIMIT 1");
          $stAv2->execute([$userId]);
          $avatarUrl = ($av2 = $stAv2->fetchColumn()) ? $av2 : null;
        ?>
        <?php if ($avatarUrl): ?>
          <img src="<?= htmlspecialchars($avatarUrl) ?>" alt="<?= $userName ?>" style="width:34px;height:34px;object-fit:cover;border-radius:99px;display:block;">
        <?php else: ?>
          <?= $initials ?>
        <?php endif; ?>
      </div>
